...
def create_graph_image(df: DataFrame) -> Image.Image:
    ...
    for day_index, (day_name, weeks) in enumerate(df.iterrows()):
        ...
        for week_col, value in enumerate(weeks):
            ...
            intensity = int(max(10, 255 * np.sqrt(contributions / (max(1, 10 - min_value)))))
    ...
    for line_no, line in enumerate(content):
        ...
        for char_col, char_value in enumerate(line):
            ...
            if not char_value.isdigit():
                raise Exception(f"Invalid value in chart file ({line_no},{char_col}): letter {char_value}")
...
def differences_between_dataframes(original: DataFrame, desired: DataFrame) -> DataFrame:
    ...
    for index, row in desired.iterrows():
        df.iloc[index, :] =  row.map(lambda x: (x[0], max(0, x[1] - original.get(x[0], 0))))
        #print("desired:\n", row)
        #print("diff:\n",df.iloc[index, :],"\n")
...
def main():
    chart_df = parse_chart_file('test.chart')
...
#   REQUIRED ARGS
    parser.add_argument('-r', '--repository', help='specify a repository where the commits will go')
    parser.add_argument('-dry', '--dry', help='doesn\'t push anything to the repository. if you won\'t specify repository it\'s implicitly a dry run')
...
#   1) GENERATE RANDOM NOISE
    ...
    parser.add_argument('-p', '--poisson', type=int,
